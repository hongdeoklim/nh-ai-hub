...
import { getGuardrailLanguageModel } from './config'

export type GuardrailVerdict = 'PASS' | 'BLOCK'

/**
 * Edge `supabase/functions/_shared/guardrail-prompt.ts` 와 동일한 문구 유지.
 * 수정 시 양쪽을 함께 갱신하세요.
 */
...
export function parseGuardrailVerdict(raw: string): GuardrailVerdict | null {
  const trimmed = raw.trim()
  if (!trimmed.length) return null

  const upper = trimmed.toUpperCase()
  const word = /\b(PASS|BLOCK)\b/.exec(upper)
  if (word) return word[1] as GuardrailVerdict
...
/**
 * 사용자 질문이 사내 업무와 관련되는지 저비용 모델로 1차 판별합니다.
 * @returns 'PASS' (통과) 또는 'BLOCK' (업무 외 요청)
 */
export async function evaluatePromptGuardrail(prompt: string): Promise<GuardrailVerdict> {
  const trimmed = prompt.trim()
  if (trimmed.length === 0) {
    return 'BLOCK'
  }

  if (quickKeywordGuardrail(trimmed) === 'BLOCK') {
    return 'BLOCK'
  }

  try {
    const { text } = await generateText({
      model: getGuardrailLanguageModel(),
