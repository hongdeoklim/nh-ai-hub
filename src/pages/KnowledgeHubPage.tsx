    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-6 md:px-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/reference-room"
            className="rounded-lg border border-stone-200 bg-white px-3 py-1.5 text-[13px] font-medium text-stone-700 hover:bg-stone-50 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-200 dark:hover:bg-stone-800"
          >
            자료실에서 문서 추가
          </Link>
          <button
            ...
            onClick={() => void reload()}
            disabled={loading}
            className="rounded-lg border border-stone-200 bg-white px-3 py-1.5 text-[13px] font-medium text-stone-700 hover:bg-stone-50 disabled:opacity-50 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-200 dark:hover:bg-stone-800"
          >
            {loading ? '불러오는 중…' : '새로고침'}
          </button>
        </div>
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              className="shrink-0 rounded-lg bg-amber-600 px-4 py-2 text-[13px] font-semibold text-white hover:bg-amber-700 disabled:opacity-50 dark:bg-amber-500 dark:text-stone-950 dark:hover:bg-amber-400"
            >
              {reembedBusy ? '재임베딩 중…' : '20건 재임베딩'}
            </button>
          </div>
          {reembedNote ? (
            <p className="mt-2 text-[12px] text-amber-900 dark:text-amber-200">
              {reembedNote}
            </p>
          ) : null}
        </section>
      ...
      >
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg border border-stone-300 bg-white px-3 py-2 text-[14px] text-stone-900 outline-none focus:border-stone-500 dark:border-stone-700 dark:bg-stone-950 dark:text-stone-100"
          />
          <button
            type="button"
            onClick={() => void handleSearchTest()}
            disabled={searchBusy}
            className="rounded-lg bg-stone-900 px-4 py-2 text-[13px] font-semibold text-white hover:bg-stone-700 disabled:opacity-50 dark:bg-stone-100 dark:text-stone-900 dark:hover:bg-stone-300"
          >
            {searchBusy ? '검색 중…' : '테스트'}
          </button>
        </div>
        ...
        {searchMatches !== null ? (
          searchMatches.length === 0 ? (
            <p className="mt-3 rounded-lg bg-stone-50 px-3 py-2 text-[13px] text-stone-500 dark:bg-stone-950 dark:text-stone-400">
              ...
              {searchMatches.map((m, i) => (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="min-w-0 truncate text-[13px] font-medium text-stone-800 dark:text-stone-200">
                      [{i + 1}] {m.fileName}
                    </p>
                  ...
                  <p className="mt-1 line-clamp-3 text-[12px] leading-relaxed text-stone-600 dark:text-stone-400">
                    {m.snippet}
                  </p>
                </li>
              ))}
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <button
            ...
            className="rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-[13px] font-medium text-red-700 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-40 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20"
          >
        ...
        {queueRows.length === 0 && !queueError ? (
          <p className="mt-3 text-[13px] text-stone-500 dark:text-stone-400">
